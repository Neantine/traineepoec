...
const server = http.createServer(function createserverCB(request, response)
{
    ...
    if (request.url === '/films')
    {
        ...
        connection.connect();
        connection.query("SELECT * FROM film", function queryCB(err, result, fields) {
                if (err) {
         ...
         <tr>
           ...
           <th>RELEASE YEAR</th>
            ...
            //response.end(JSON.stringify(rows)); Bien pour debugger avant ttt HTML
    ...
    else if (new RegExp('\\/films\\/\\d+$').test(request.url))   //Utiliser http://regexr.com/
    {
        ...
        connection.connect();
        //Attention !!! Faille n1 de securité (injection SQL potentielle), escaping avec ? et [] (offert par la lib mysql)
        connection.query("SELECT * FROM film WHERE film_id = ?", [film_id], function queryCB(err, result, fields)
        {
            if (err)
            {
                console.log(err); //Ici on peut logger les erreurs pour debugger mais surtout pas dans la reponse http (securité)!!!!
